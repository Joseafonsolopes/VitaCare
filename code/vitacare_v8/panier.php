  ...
  function Panier() {
    ...
    function confirmerPaiement() {
      ...
      let promesses = panier.map(function(item) {
        ...
        formData.append('nb_places', item.nb_places || 1);
      ...
      Promise.all(promesses).then(function(resultats) {
        ...
        if (erreurs.length === 0) {
          const factures = resultats.map(function(r) { return r.numero_facture; });
      ...
      return (
        <div className="panier-page">
          <div className="confirmation-bloc">
            ...
            {factures.length > 0 && (
              <div className="factures-confirmation">
                <p style={{fontWeight:'600', marginBottom:'8px'}}>📄 Vos numéros de facture :</p>
                {factures.map(function(f, i) {
                  return <p key={i} style={{fontSize:'13px', color:'var(--vert-fonce)', fontFamily:'monospace'}}>{f}</p>;
                })}
              </div>
            )}
            <div className="confirmation-actions">
              <a href="profil.php" className="btn-primaire">Voir mes réservations</a>
              <a href="profil.php#factures" className="btn-secondaire" style={{marginLeft:'12px'}}>Voir mes factures</a>
            </div>
          </div>
        </div>
      );
                ...
                return (
                  <div key={i} className="panier-item">
                    ...
                    <div className="panier-item-info">
                      <p className="panier-item-nom">{item.nom_service} {item.nb_places > 1 ? <span className="badge-places">×{item.nb_places}</span> : ''}</p>
          ...
          {panier.length > 0 && (
            <div className="panier-recap">
              ...
              {panier.map(function(item, i) {
                return (
                  <div key={i} className="recap-ligne">
                    <span>{item.nom_service} {item.nb_places > 1 ? '×' + item.nb_places : ''}</span>
                    <span>{item.prix}€</span>
                  </div>
                );
              })}
