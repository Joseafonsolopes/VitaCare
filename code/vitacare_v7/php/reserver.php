...
if (mysqli_query($conn, $sql_resa)) {
  ...
  // Notification améliorée avec nom du service et téléphone intervenant
  $sql_info = "SELECT s.nom, u.prenom, u.nom AS nom_intervenant, u.telephone, c.date_creneau, c.heure
  ...
  $nom_service    = $info['nom'];
  $prenom_interv  = $info['prenom'];
  $nom_interv     = $info['nom_intervenant'];
  $tel_interv     = $info['telephone'] ? $info['telephone'] : 'non renseigné';
  $date_creneau   = $info['date_creneau'];
  $heure_creneau  = $info['heure'];

  $message_notif = "Merci de participer à \"$nom_service\" le $date_creneau à $heure_creneau. Pour toute question, contactez $prenom_interv $nom_interv au $tel_interv.";

  $sql_notif = "INSERT INTO notifications (id_utilisateur, message, lu, date_notification)
                VALUES ('$id_utilisateur', '$message_notif', 0, NOW())";
